...
module.exports = {
  headerValidator: (req) => {
    if (
      req.header("X-AccessKey") ==
        lehavaData.restaccess.rest_access.access_key &&
  ...
  basicHeaderValidator: (req) => {
    if (
      ...
      req.header("Accept") == "application/json" &&
      req.header("Authorization") == "Basic c2VydmljZWRlc2s6U0RBZG1pbjAx"
    ) {
      return true;
    }
    return false;
  ...
  userCallsHeaderValidator: (req) => {
    if (req.header("X-Obj-Attrs")) {
      if (
        req.header("X-Obj-Attrs") ==
        "status, summary, description, open_date, z_network, z_impact_service, group, web_url, type, active"
      ) {
        return true;
  ...
  servicesValidator: (req) => {
    if (req.query.WC) {
      if (
        req.query.WC.startsWith("network=U'") &&
        req.header("X-Obj-Attrs") == "service"
      ) {
        return true;
      }
      return false;
  ...
  updatesValidator: (req) => {
    if (
      req.header("Content-Type") == "application/json" &&
      req.header("X-Obj-Attrs") ==
        "category, description, open_date, summary, z_network"
    ) {
